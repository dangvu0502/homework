...
export const AnnotationCanvas = ({
  ...
  const drawCanvas = useCallback(() => {
    ...
    boundingBoxes.forEach((box) => {
      ...
      ctx.strokeStyle = tagColors[box.tag];
      ctx.fillStyle = tagColors[box.tag] + (isHighlighted ? '40' : '20');
      ctx.lineWidth = isHighlighted ? 3 : 2;
      
      if (isHighlighted) {
        // Add glow effect for highlighted annotation
        ctx.shadowColor = tagColors[box.tag];
        ctx.shadowBlur = 8;
      }
      ...
      // Draw tag label
      ctx.fillStyle = tagColors[box.tag];
    ...
    <div className={`space-y-4 ${className}`}>
      <Card className="p-4">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground mt-2">
          ...
          <span>Current tag: <strong>{selectedTag}</strong></span>
        </div>
      </Card>
    </div>
  );
